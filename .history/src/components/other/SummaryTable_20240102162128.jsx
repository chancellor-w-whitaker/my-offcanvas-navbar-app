...
const initActiveColumns = new Set(["termDesc"]);
...
export const SummaryTable = () => {
  ...
  const [dropdownState, setActiveColumns] = useState(initActiveColumns);
  ...
  const filteredColumnDefs = useMemo(
    () =>
      columnDefs.filter(
        ({ field }) => dropdownState.has(field) || field === activeMeasure
      ),
    [columnDefs, dropdownState, activeMeasure]
  );
  ...
  const onDropdownItemClick = useCallback(
    (e) =>
      startTransition(() =>
        setActiveColumns((previousState) => {
