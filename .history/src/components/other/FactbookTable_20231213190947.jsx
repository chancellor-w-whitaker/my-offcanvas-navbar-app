...
export const FactbookTable = () => {
  ...
  const { location: fetchUrl } = datasets.find(({ id }) => id === activeTabID);
  ...
  const onTabBgTransitionEnd = useCallback(
    ({ propertyName }, tabID) => {
      if (propertyName === "background-color" && tabID === activeTabID) {
        fetch(fetchUrl)
          .then((resp) => resp.json())
  ...
  return (
    <>
      <DatasetTabs
        onTabBgTransitionEnd={onTabBgTransitionEnd}
