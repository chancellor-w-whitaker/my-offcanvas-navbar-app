...
import { getEachColTypeOccurrences } from "../../functions/getEachColTypeOccurrences";
...
export const SummaryTable = () => {
  ...
  const columnDefs = useMemo(() => {
    const eachColTypeOccurrences = getEachColTypeOccurrences(rowData);

    const colDefs = Object.entries(eachColTypeOccurrences).map(
    ...
    const stringCols = colDefs.filter((object) => !("type" in object));

    const numberCols = colDefs.filter((object) => "type" in object);

    return [
      [...stringCols, ...numberCols],
      stringCols.map(({ field }) => field),
      new Set([stringCols[0]]),
    ];
  }, [rowData]);
