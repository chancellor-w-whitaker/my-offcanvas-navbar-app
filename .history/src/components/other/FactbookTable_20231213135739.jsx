...
export const FactbookTable = () => {
  ...
  const rowDataLocation = activeDataset.location;

  const deferredRowDataLocation = useDeferredValue(rowDataLocation);

  const columnDefs = useMemo(() => {
    const firstRow = rowData.length > 0 ? rowData[0] : {};
  ...
  const onGridReady = useCallback(() => {
    fetch(deferredRowDataLocation)
      .then((resp) => resp.json())
      .then((data) => setRowData(data));
  }, [deferredRowDataLocation]);
