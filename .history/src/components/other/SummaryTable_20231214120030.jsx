...
const getColumnTypes = (rowData) => {
  const columnTypes = {};

  Array.isArray(rowData) &&
    rowData.forEach((row) =>
      Object.entries(row).forEach(([field, value]) => {
        if (!(field in columnTypes)) columnTypes[field] = {};

        if (value || value === 0) {
          if (!(typeof value in columnTypes[field]))
            columnTypes[field][typeof value] = 0;

          columnTypes[field][typeof value]++;
        }
      })
    );
...
const findMostCommonType = (typeOccurrenceObject) => {
  return Object.entries(typeOccurrenceObject).sort(
...
export const SummaryTable = () => {
  ...
  const columns = useMemo(() => {
    const columnTypes = getColumnTypes(rowData);

    const defs = Object.entries(columnTypes).map(([field, types]) => ({
      type: Object.entries(types).sort(
        ([typeA, numberOfOccurrencesA], [typeB, numberOfOccurrencesB]) =>
          numberOfOccurrencesB - numberOfOccurrencesA
      )[0][0],
      field,
