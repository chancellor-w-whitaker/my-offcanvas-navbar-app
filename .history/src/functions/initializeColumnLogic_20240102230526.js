...
export const initializeColumnLogic = (rows, pivotColumn) => {
  ...
  const setOfPivotValues = new Set();

  rows.forEach((row) => setOfPivotValues.add(row[pivotColumn]));

  // ! model of numeric column def found in typedColumnDefs assignment
  const pivotColumnDefs = [...setOfPivotValues].map((field) => ({
    valueFormatter: ({ value }) => Math.round(value).toLocaleString(),
    headerName: toTitleCase(field),
    type: "numericColumn",
    field,
  ...
  const initialSummaryColumns = new Set([summaryColumnsList[0]]);
